...
function PtyReadStream(term, options) {
	...
	handle.open(this.master_fd);
	options = extend({}, options, {
		handle: handle,
		readable: true,
		writeable: false
	});
	PtyReadStream.super_.call(this, options);
}
util.inherits(PtyReadStream, net.Socket);

PtyReadStream.prototype.close = function(cb) {
	var self = this;
	if(cb)
		this.once('close', cb);
	fs.close(this.master_fd, function(err) {
		if (err)
			self.emit('error', err);
		else
			self.emit('close');
...
module.exports.spawn = function(command, args, options) {
	...
	child.stdio.pop().on('data', function(data) {
